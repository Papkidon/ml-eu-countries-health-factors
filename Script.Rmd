---
title: "Comparision of chosen UE countries in regards to health factors"
output: html_document
author: Paweł Kopaniecki
---
...
# Prerequisites
```{r bibinstall, message = F, warning = F}
library(devtools)
...
# Load packages :
```{r bibload, message = F, warning=FALSE}
require(readxl)
require(dplyr)
require(xlsx)
library(devtools)
...
```{r map_countries, warning=F, message=F, echo=F}
...
# Boxplot of raw data
```{r df_boxplot, message=F, warning=F}
...
```{r df_basic_plots, warning=F, message=F, echo = F}
...
```{r hist_raw_1, message - F, warning = F}
...
```{r chosen_var, message = FALSE, warning = FALSE}
...
# Standardization
```{r df_stand, message = F, warning=F}
df.stand <- scale(df)
...
```{r df_stand_boxplot, warning=F, message=F}
...
```{r fviz_elbow_1, message = F, warning = F}
...
```{r fviz_gap_1, message = F, warning = F}
...
```{r kmeans_alg_2, message = F, warning = F}
...
```{r kmeans_alg_3, message = F, warning = F}
...
```{r test_s, warning=F, message=F}
index.S(dist(df.stand), kme3$cluster) #centers = 3, score = 0.26
index.S(dist(df.stand), kme2$cluster) #centers = 2, score = 0.43 <- better
```

### Index G1 test
```{r test_g1, warning=F, message=F}
index.G1(df.stand, kme3$cluster) #centers = 3, wynik = 14.52
index.G1(df.stand, kme2$cluster) #centers = 2, wynik = 16.9 <- better
```

### Index G2 test
```{r test_g2, warning=F, message=F}
index.G2(dist(df.stand), kme3$cluster) #centers = 3, wynik = 0.757
index.G2(dist(df.stand), kme2$cluster)  #center = 3, wynik = 0.847 <- better
```
...
```{r pca_alg, warning=F, message=F}
DFPCA <- PCA(df.stand, scale.unit = FALSE, ncp = 2, graph = F)
```

## Eigenvalues
```{r pca_eigval, warning=F, message=F}
DFPCA.eigval <- get_eigenvalue(DFPCA)
DFPCA.eigval
```

## Eigenvalues plot
```{r pca_eigval_plot, warning=F, message=F}
...
```{r pca_corrplot, warning=F, message=F}
...
```{r pca_ind_dim1, warning=F, message=F}
fviz_contrib(DFPCA,
             choice = "var",
             axes = 1,
             top = 14,
...
```{r pca_corr_pval, warning=F, message=F}
...
```{r pca_contrib_all, warning=F, message=F}
fviz_contrib(DFPCA,
             choice = "ind",
             axes = 1:2,
...
```{r map_clustered, warning=F, message=F, echo=F}
